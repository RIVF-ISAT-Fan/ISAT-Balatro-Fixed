  ...
  loc_vars = function(self,info_queue,card)
    if card.config then
      if card.ability.extra.phase < 3 and not card.debuff then
        info_queue[#info_queue + 1] = {generate_ui = isat_tooltip, set='ISAT', key = 'timeloop', title = 'Your Wish?',vars = {(card.ability.extra.loop and "Active") or "Inactive"}}
      end
...
SMODS.Joker{
  ...
  loc_vars = function(self,info_queue,card)
    if card.ability.extra.rice then
      info_queue[#info_queue + 1] = {generate_ui = isat_tooltip, set='ISAT', key = 'onigiri2', title = 'Good Snack'}
    end
  end,
  calculate = function(self,card,context)
